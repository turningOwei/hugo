...
function loadSample() {
    ...
    var sampleData = [
        ...
        { "id": 2, "name": "李四", "email": "lisi@example.com", "age": 32, "department": "市场部", "skills": ["营销", "数据分析"] },
        { "id": 3, "name": "王五", "email": "wangwu@example.com", "age": 25, "department": "设计部", "skills": ["UI设计", "Figma"] }
    ];
...
function renderTableView(data) {
    ...
    // Only arrays of objects can be shown as tables
...
function buildTreeHtml(data, indent, isRoot) {
    ...
    if (Array.isArray(data)) {
        ...
        data.forEach(function(item, i) {
            html += '<div><span class="json-tree-key">' + i + '</span>: ' + buildTreeHtml(item, indent + '  ', false) + '</div>';
        });
        html += '</div>';
        return html;
    }
    ...
    if (typeof data === 'object') {
        ...
        html += '<div class="json-tree-node">';
        keys.forEach(function(key) {
            html += '<div><span class="json-tree-key">"' + escapeHtml(key) + '"</span>: ' + buildTreeHtml(data[key], indent + '  ', false) + '</div>';
        });
        html += '</div>';
        return html;
    }
